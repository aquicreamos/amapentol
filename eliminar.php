...
$query = "SELECT afiliados.idCedula, afiliados.Apellido, afiliados.segApellido, afiliados.FNacimiento, afiliados.Nombre, afiliados.Direccion, afiliados.Telefonofijo, afiliados.Municipio, afiliados.TelCelular, afiliados.email, afiliados.nicename, afiliados.contrasenia, bancos.BancoName, categoriasuser.Ncategoria, clase.ClaseName, departamentos.nombre_departamento, municipios.nombre_municipio, salud.SaludName, activo.tipo FROM afiliados , bancos , categoriasuser , clase , departamentos , municipios , salud , activo WHERE afiliados.idCedula = '$idcedula' AND municipios.id = afiliados.municipios_id AND departamentos.id_dep = afiliados.departamentos_id AND salud.idSalud = afiliados.Salud_idSalud AND bancos.idBancos = afiliados.Bancos_idBancos AND categoriasuser.idCategoriasuser = afiliados.idCategoriasuser AND clase.idClase = afiliados.Clase_idClase AND activo.idactivo = afiliados.activo_idactivo" or die("Error in the consult.." . mysqli_error($link)); 
...
<!--Ingreso de Cedula-->
					<div class="form-group">
						...
						<div class="col-lg-10" >
						<input type="text" class="form-control" id="inputCedula" placeholder="<?php echo $idcedula;?>" name="fcedula" disabled>
...
<form role="form" action="eliminaraceptado.php">
    <button type="submit" class="btn btn-danger" aling="center"><h5>Eliminar</h5></button>
</form>
